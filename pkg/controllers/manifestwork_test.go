...
func getPayloadInManifestwork(payload []workv1.Manifest, kind string) *unstructured.Unstructured {
...
func TestManifestWorkFlowWithSSHKeyInProviderSecret(t *testing.T) {
	client := initClient()
	ctx := context.Background()
	hdr := &HypershiftDeploymentReconciler{
		Client: client,
	...
	cloudProviderSecret := &corev1.Secret{
		ObjectMeta: metav1.ObjectMeta{
			Name:      testHD.Spec.Infrastructure.CloudProvider.Name,
			Namespace: testHD.Namespace,
		},
		Data: map[string][]byte{
			constant.SSHPublicKey:  []byte("public-key"),
			constant.SSHPrivateKey: []byte("private-key"),
		},
	}
	err := client.Create(ctx, cloudProviderSecret)
	defer client.Delete(ctx, cloudProviderSecret)
	...
	// Check manifestwork.manifest has the SSH secret
	manifestWork := &workv1.ManifestWork{}
	...
	usHcFromManifest := getPayloadInManifestwork(manifestWork.Spec.Workload.Manifests, "HostedCluster")
	...
	assert.Equal(t, sshKeySecretName, hcSpecFromManifest["sshKey"].(map[string]interface{})["name"], "equals when the ssh key exists in the hosted cluster")
}
...
func TestManifestWorkHostedClusterAttributes(t *testing.T) {
	...
	// Check manifestwork.manifest has hostedcluster.spec with the new attribute
	usNpFromManifest := getPayloadInManifestwork(manifestWork.Spec.Workload.Manifests, "NodePool")
